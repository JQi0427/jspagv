...
for i in range(num):
    ...
    if init_sequence[temp_job] != 0 & init_sequence[temp_job] != 5:  # 该工序不是第一步工序也不是最后一步工序
        ...
        if boolean_agv[temp_agv] == 0:  # 判断agv是否空闲,0:空闲
            if init_time[temp_machine] > init_time[last_machine-1]:  # 判断该工件的上一步工序是否完成，完成
            ...
            else:  # 判断该工件的上一步工序是否完成，未完成
                time = init_time[last_machine-1] - init_time[temp_machine]
                if time > agv[location_agv[temp_agv]][last_machine]:  # 上一步完成加工的时间>agv从原来的位置移动目前作业所在的机器
                    init_time[temp_machine] += pt[temp_job][init_sequence[temp_job]] + \
                                               agv[last_machine][temp_machine + 1] + time
                else:
                    init_time[temp_machine] += pt[temp_job][init_sequence[temp_job]] + agv[last_machine][temp_machine + 1] + \
            ...
            machine = location_agv[temp_agv] - 1  # 得到agv完成上个任务所在的机器，用那个机器的上的时间减去pt，得出agv到达上个任务机器的时间
